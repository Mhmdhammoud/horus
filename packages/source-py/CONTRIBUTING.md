...
2. Create `src/horus_source/core/ingestion/languages/<lang>.py` following the pattern in `python.py` or `typescript.py`
3. Register the language in `src/horus_source/core/ingestion/languages/__init__.py`
4. Add tree-sitter grammar dependency to `pyproject.toml`
5. Write tests in `tests/core/test_parser_<lang>.py`
6. Verify integration: `horus-source analyze <test-repo>` should index the new language
