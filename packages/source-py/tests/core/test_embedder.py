...
def test_embed_thread_count_pinned_and_overridable() -> None:
    ...
    try:
        assert _embed_thread_count() == 4
        os.environ["HORUS_EMBED_THREADS"] = "2"
        assert _embed_thread_count() == 2
        os.environ["HORUS_EMBED_THREADS"] = "garbage"
        assert _embed_thread_count() == 4
    finally:
        os.environ.pop("HORUS_EMBED_THREADS", None)
        if saved is not None:
            os.environ["HORUS_EMBED_THREADS"] = saved
...
class TestModelDefaults:
    ...
    def test_default_batch_size(self) -> None:
        # Lowered from 32 to bound embedding peak memory: attention activations
        # grow with batch_size * seq_len^2, so a large batch over long code texts
        # could balloon to many GB and OOM the indexer. 8 keeps peak in the low GBs.
...
class TestEmbedGraphTextGeneration:
    @patch("horus_source.core.embeddings.embedder.generate_text")
    @patch("fastembed.TextEmbedding")
    ...
    def test_generated_texts_passed_to_model(
        ...
        # The texts passed to model.passage_embed should be both generated texts, each
        # carrying the nomic "search_document: " task prefix (the default model is nomic).
        ...
        assert "search_document: text for foo" in texts_arg
        assert "search_document: text for Bar" in texts_arg
...
class TestEmbedGraphBatchProcessing:
    ...
    @patch("fastembed.TextEmbedding")
    def test_default_batch_size_is_8(self, mock_te_cls: MagicMock, sample_graph: KnowledgeGraph) -> None:
        mock_model = MagicMock()
        mock_model.passage_embed.return_value = iter(
            [_vec768([0.1, 0.2, 0.3]), _vec768([0.4, 0.5, 0.6])]
        )
        ...
        # Default batch size lowered to 8 to bound embedding peak memory.
        embed_call = mock_model.passage_embed.call_args
...
class TestEmbedQueryNomic:
    ...
    @patch("fastembed.TextEmbedding")
    def test_query_carries_nomic_search_query_prefix(self, mock_te_cls):
        # nomic requires the "search_query: " task prefix; without it the query/document
        # asymmetry the model was trained on is lost and semantic recall degrades.
        mock_model = MagicMock()
        mock_model.query_embed.return_value = iter([np.array([0.5] * 768)])
        mock_te_cls.return_value = mock_model
        embed_query("deduplicate incoming leads")
        call = mock_model.query_embed.call_args
        passed = call.args[0] if call.args else call.kwargs.get("query")
        assert passed == "search_query: deduplicate incoming leads"
