...
class TestHumanize:
    def test_splits_camel_case(self):
        assert _humanize("markDuplicateLead") == "mark duplicate lead"

    def test_splits_snake_and_kebab_and_dotted(self):
        assert _humanize("mark_duplicate_lead") == "mark duplicate lead"
        assert _humanize("mark-duplicate-lead") == "mark duplicate lead"
        assert _humanize("order.created") == "order created"

    def test_splits_acronym_boundaries(self):
        assert _humanize("HTTPServer") == "http server"
        assert _humanize("HTTP2Server") == "http2 server"

    def test_empty_when_nothing_gained(self):
        # A single lowercase word adds no new signal over the exact token.
        assert _humanize("create") == ""
        assert _humanize("") == ""
        assert _humanize(None) == ""


class TestHeaderHumanization:
    def test_method_text_leads_with_humanized_words(self):
        graph = KnowledgeGraph()
        m = _node(
            NodeLabel.METHOD,
            "markDuplicateLead",
            file_path="src/zoho/pusher.ts",
            class_name="ZohoPusherService",
            signature="markDuplicateLead(args: ClaimedRow)",
        )
        graph.add_node(m)
        text = generate_text(m, graph, build_class_method_index(graph))
        # Natural-language words present for the embedding model to latch onto...
        assert "mark duplicate lead" in text
        # ...and the exact identifier is still present for lexical/FTS overlap.
        assert "markDuplicateLead" in text
