...
from horus_source.core.ingestion.walker import discover_files, is_minified, walk_repo
...
class TestMinifiedDetection:
    """HOR-352: minified/bundled assets must be skipped by content, not just name."""

    def test_is_minified_true_for_long_line(self) -> None:
        assert is_minified("var x=" + "a" * 6000 + ";") is True

    def test_is_minified_false_for_normal_source(self) -> None:
        assert is_minified("def main():\n    return 1\n") is False

    def test_walk_repo_skips_minified_by_content(self, tmp_path: Path) -> None:
        # Innocuous name that matches no ignore glob, but minified content (one
        # 8000-char line) — like a webpack bundle. Must be excluded by content.
        (tmp_path / "app.js").write_text(
            "(function(){" + "x=1;" * 2000 + "})();", encoding="utf-8"
        )
        (tmp_path / "real.js").write_text("export const a = 1;\n", encoding="utf-8")
        paths = {e.path for e in walk_repo(tmp_path)}
        assert "real.js" in paths
        assert "app.js" not in paths

    def test_walk_repo_skips_hyphen_bundle_by_name(self, tmp_path: Path) -> None:
        # e.g. swagger-ui-bundle.js — matches the broadened *-bundle.js glob.
        (tmp_path / "swagger-ui-bundle.js").write_text("ok();\n", encoding="utf-8")
        (tmp_path / "real.js").write_text("export const a = 1;\n", encoding="utf-8")
        paths = {e.path for e in walk_repo(tmp_path)}
        assert "real.js" in paths
        assert "swagger-ui-bundle.js" not in paths
