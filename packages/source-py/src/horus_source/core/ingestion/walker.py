...
from horus_source.config.ignore import DEFAULT_IGNORE_PATTERNS, should_ignore
from horus_source.config.languages import get_language, is_supported
...
_PRUNE_DIRS: frozenset[str] = frozenset(
    ...
    ".ruff_cache", ".tox", ".eggs", ".horus",
})

# A source line longer than this almost never occurs in hand-written code, but
# is the defining trait of minified / bundled / generated assets (e.g. a 1.4 MB
# webpack `swagger-ui-bundle.js` packed onto one line). Such files pollute the
# code graph with garbage symbols, so we skip them by *content* — independent of
# file name — which name-based ignore globs cannot reliably catch.
MINIFIED_MAX_LINE_LEN = 5000


def is_minified(content: str) -> bool:
    """Heuristic: True when *content* looks minified/generated (a very long line)."""
    longest = 0
...
def read_file(repo_path: Path, file_path: Path) -> FileEntry | None:
    ...
    # Skip minified/bundled/generated assets by content (HOR-352): name-based
    # globs miss things like `swagger-ui-bundle.js`, and indexing them floods the
    # graph with junk symbols and seeds.
