"""MCP resource handlers for Horus.
...
def get_overview(storage: StorageBackend) -> str:
    ...
    """
    lines = ["Horus Codebase Overview", "=" * 40, ""]

    try:
        node_counts = storage.count_nodes_by_label()
        if node_counts:
    ...
    try:
        edge_counts = storage.count_edges_by_type()
        if edge_counts:
...
def get_dead_code_symbols(storage: StorageBackend) -> list:
    ...
    """
    return storage.get_dead_code_symbols()
...
def get_schema() -> str:
    """Return a static description of the Horus knowledge graph schema.
    ...
    Returns:
        ...
        and key properties.
    """
    return """Horus Knowledge Graph Schema
========================================
